...
module.exports = async (logger, sampleId, transcriptInput, fusionInput, output) => {
  ...
  if (transcriptInput) {
    await reader(transcriptInput, line => {
      ...
      stream.write(`${sampleId},${formatGene(GENE)},N/A,fusion_transcript,${left_chr},${left_pos},${left_pos},${right_chr},${right_pos},${right_pos},unknown,somatic,N/A,"${JSON.stringify(attributes).replace(/"/g, '\'')}"\n`);
    });
  }
  ...
  if (fusionInput) {
    await reader(fusionInput, line => {
      ...
      stream.write(`${sampleId},${formatGene(gene1)},${formatGene(gene2)},fusion,${left_chr},${left_pos},${left_pos},${right_chr},${right_pos},${right_pos},unknown,somatic,N/A,"${JSON.stringify(attributes).replace(/"/g, '\'')}"\n`);
    });
  }
