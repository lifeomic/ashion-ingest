...
test.beforeEach(async t => {
  try {
    fs.unlinkSync(path.join(__dirname, 'report.json'));
  } catch (ignored) {}
});

test.serial('The report function will save a DiagnosticReport FHIR resource', async t => {
  await report(console, 'project', 'patient', 'https://api.com/v1/files/1234', '2000-01-01', path.join(__dirname, 'report.json'));

  const result = fs.readFileSync(path.join(__dirname, 'report.json'), 'utf8');
  t.is(result,
